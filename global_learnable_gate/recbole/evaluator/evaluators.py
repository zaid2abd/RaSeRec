...
class TopKEvaluator(GroupedEvaluator):
    ...
    def get_gini(self, item_matrix, num_items=23033):
    ...
    def evaluate(self, batch_matrix_list, eval_data):
        ...
        gini = self.get_gini(torch.cat(batch_matrix_list, dim=0).cpu().numpy()[:, :5])
        shon = self.get_entropy(torch.cat(batch_matrix_list, dim=0).cpu().numpy()[:, :5])
        print("############gini:{}, entro: {}###########".format(gini, shon))
        return metric_dict
